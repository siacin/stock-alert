...
test('detailed agent result renders classifications and escapes model text',()=>{
  ...
  f.run(`renderNewsAgentResult(${JSON.stringify({structured:true,metadata:{model:'test',news_count:1,manual_news_provided:true},analysis:{
    ...
    validation_signals:{confirmed:['政策发布'],to_verify:['订单'],invalidation:['政策撤回']},scenarios:[{name:'基准',probability:'中',conditions:['执行落地'],market_impact:'板块分化'}],coverage_audit:{directions_checked:['设备'],candidate_count:12,included_count:2,excluded_count:1,unresolved_categories:['材料'],deferred_topics:[{title:'消费政策',reason:'本轮预算'}]},topic_results:[{topic_id:'u-failed',title:'独立失败方向',ok:false,error:'该方向流式连接中断',analysis:{}}],risks:['不构成建议']
  }})})`);
  const html=f.node('#newsAgentResult').innerHTML;
  for(const label of ['集合主题总览','本轮简析','事件画像','单方向深挖','弹性公式','逻辑传导链','关联板块','主攻 · 逻辑最强','观察 · 强关联待验证','排除项','板块梯队','事实核验','情景推演','覆盖审计','本轮未深挖','逐方向独立分析','该方向未影响其他结果']) assert.match(html,new RegExp(label));
