...
function renderDetailedAgentSections(analysis) {
  ...
  const logicClosure = agentArray(analysis?.logic_closure);
  const closureSummary = analysis?.logic_closure_summary && typeof analysis.logic_closure_summary === "object" ? analysis.logic_closure_summary : null;
  const thesisBalance = analysis?.thesis_balance && typeof analysis.thesis_balance === "object" ? analysis.thesis_balance : null;
  const marketPricing = analysis?.market_pricing && typeof analysis.market_pricing === "object" ? analysis.market_pricing : null;
  const evidenceLedger = agentArray(analysis?.evidence_ledger);
  ...
  const signalBoard = analysis?.signal_board && typeof analysis.signal_board === "object" ? analysis.signal_board : {};
  const decisionChain = agentArray(analysis?.decision_chain);
  ...
  if (!profile && !topics.length && !logicClosure.length && !closureSummary && !deepDives.length && !paths.length && !sectors.length && !bucketCount && !ladders.length && !scenarios.length) return "";
  ...
  return `
    ...
    ${(logicClosure.length || closureSummary) ? `<section class="agent-result-section"><h4>逻辑闭环审计 · 变化 → 影响 → 业绩 → 股价定价</h4>
      ${closureSummary ? `<div class="agent-closure-summary"><div class="agent-profile-tags"><span>${agentText(closureSummary.grade, "待评级")}级闭环</span><span>事件：${agentText(closureSummary.event_stage, "待核验")}</span><span>定价：${agentText(closureSummary.pricing_stage, "待核验")}</span></div><p>${agentText(closureSummary.overall, "尚未形成闭环结论")}</p><small>最弱环节：${agentText(closureSummary.weakest_link, "待识别")}<br>升级所需证据：${agentText(closureSummary.next_upgrade_condition, "待补充")}</small></div>` : ""}
      ${logicClosure.length ? `<div class="agent-closure-grid">${logicClosure.map((item, index) => `<article><div><b>${agentText(item?.stage, `阶段 ${index + 1}`)}</b><span>${agentText(item?.status, "待核验")}</span></div><p>${agentText(item?.conclusion, "暂无结论")}</p>${agentMiniList("支撑证据", item?.evidence)}${agentMiniList("缺口/反向条件", item?.missing_or_risks, "warning")}</article>`).join("")}</div>` : ""}
    </section>` : ""}
    ${thesisBalance ? `<section class="agent-result-section"><h4>多空论证与最大分歧</h4><div class="agent-thesis-grid"><article class="bull"><b>最强正方逻辑</b><p>${agentText(thesisBalance.bull_case, "待分析")}</p></article><article class="bear"><b>最强反方逻辑</b><p>${agentText(thesisBalance.bear_case, "待分析")}</p></article></div><div class="agent-counterargument"><b>最可能推翻主逻辑的反证</b><p>${agentText(thesisBalance.strongest_counterargument, "待识别")}</p>${agentMiniList("解决分歧所需数据", thesisBalance.resolution_data, "warning")}</div></section>` : ""}
    ${marketPricing ? `<section class="agent-result-section"><h4>当前市场定价检查</h4><div class="agent-market-pricing"><div class="agent-profile-tags"><span>${agentText(marketPricing.stage, "待核验")}</span><span>数据：${agentText(marketPricing.data_status, "缺失")}</span><span>${agentText(marketPricing.captured_at, "无行情时间")}</span></div><p>${agentText(marketPricing.event_price_alignment, "尚无法判断事件与盘面是否共振")}</p><div class="agent-fact-grid">${agentMiniList("板块证据", marketPricing.sector_evidence)}${agentMiniList("龙头/梯队证据", marketPricing.leader_evidence)}${agentMiniList("拥挤与分歧", marketPricing.crowding_evidence, "warning")}${agentMiniList("数据限制", marketPricing.limitations, "warning")}</div></div></section>` : ""}
    ${evidenceLedger.length ? `<section class="agent-result-section"><h4>关键证据账本 · 结论可追溯</h4><div class="agent-evidence-ledger">${evidenceLedger.map((item) => `<article><div><b>${agentText(item?.claim, "关键判断")}</b><span>${agentText(item?.status, "待核验")}</span></div><p>${agentText(item?.evidence, "暂无证据")}</p><small>${agentText(item?.supports, "未分类")} · ${agentText(item?.source_type, "来源待核验")}</small></article>`).join("")}</div></section>` : ""}
    ...
    ${(agentArray(signalBoard.verify).length || agentArray(signalBoard.falsify).length || agentArray(signalBoard.next_catalysts).length) ? `<section class="agent-result-section"><h4>动态信号板 · 如何验证或推翻</h4><div class="agent-signal-board">
      ${agentArray(signalBoard.verify).map((item) => `<article class="verify"><b>验证｜${agentText(item?.signal, "待定义")}</b><p>${agentText(item?.upgrade_effect, "待说明升级影响")}</p><small>数据源：${agentText(item?.data_source, "待确定")}</small></article>`).join("")}
      ${agentArray(signalBoard.falsify).map((item) => `<article class="falsify"><b>证伪｜${agentText(item?.signal, "待定义")}</b><p>${agentText(item?.downgrade_effect, "待说明降级影响")}</p><small>数据源：${agentText(item?.data_source, "待确定")}</small></article>`).join("")}
      ${agentArray(signalBoard.next_catalysts).map((item) => `<article class="catalyst"><b>催化｜${agentText(item?.event, "待定义")}</b><p>${agentText(item?.what_to_watch, "待观察")}</p><small>窗口：${agentText(item?.window, "待定")}</small></article>`).join("")}
    </div></section>` : ""}
    ${decisionChain.length ? `<section class="agent-result-section"><h4>决策链 · 研究条件而非买卖指令</h4><div class="agent-decision-chain">${decisionChain.map((item, index) => `<article><i>${index + 1}</i><div><b>${agentText(item?.step, "研究步骤")}</b><p>${agentText(item?.conclusion, "待分析")}</p></div><span>${agentText(item?.evidence_status, "待核验")}</span></article>`).join("")}</div></section>` : ""}
